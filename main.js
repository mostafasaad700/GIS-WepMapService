...
// انواع الخرابط
var wayback20512  = L.tileLayer("https://wayback.maptiles.arcgis.com/arcgis/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}", {
...
var baseMaps =
{
  "Esri_satallite 30/5/2025": wayback20512 ,
  "Open street Map": osm,
...
fetch('./zmams.json')
  ...
  .then(data => {
    ...
    zmamsLayer = L.geoJSON(data, {
      onEachFeature: function (feature, layer) {
        if (feature.properties) {
          var popup = `<div style="width:1000px; max-width:100%;">`;
          popup += `<table class="table table-bordered table-sm text-center mb-0">`;
          popup += '<thead class="table-primary"><tr>';

          for (var k in feature.properties) {
            popup += `<th>${k}</th>`;
          }
          popup += '</tr></thead><tbody><tr>';

          for (var k in feature.properties) {
            popup += `<td>${feature.properties[k]}</td>`;
          }
    ...
    layerControl.addOverlay(zmamsLayer, "ZMAMS Layer");
  });
// ==================== JSON 2 ====================
var egyptLayer;
fetch("./education.json")
  ...
  .then(data => {
    ...
    layerControl.addOverlay(egyptLayer, "Egypt Layer");
  });
// ==================== Layer Control ====================
...
legend.onAdd = function () {
  ...
  div.innerHTML = `
    <h6 class="text-center mb-1">Legend</h6>
    <div><span style="background:#ff0000; width:20px; height:10px; display:inline-block;"></span> ZMAMS Layer</div>
    <div><span style="background:#00ff00; width:20px; height:10px; display:inline-block;"></span> Egypt Layer</div>
  `;
  return div;
};
legend.addTo(map);
...
values.forEach((val, index) => {
  ...
  if (isNaN(meters)) {
    row.innerHTML = `<td>${index + 1}</td>
      <td colspan="4" class="table-danger">⚠️ "${val}" ليست قيمة رقمية</td>`;
...
// الصف النهائي — الإجمالي
const totalRow = document.createElement("tr");
totalRow.classList.add("table-da");

totalRow.innerHTML = `
  <td colspan="1" class="table-warning fw-bold text-center">الإجمالي</td>
  <td class="table-warning">${totalMeters}</td>
  <td class="table-warning">${totalFaddan}</td>
  <td class="table-warning">${totalQerat}</td>
  <td class="table-warning">${totalSahm.toFixed(2)}</td>
`;
